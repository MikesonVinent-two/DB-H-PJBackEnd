...
-- 33. answer_scores (回答分数表)
DROP TABLE IF EXISTS `ANSWER_SCORES`;

CREATE TABLE `ANSWER_SCORES` (
    `ID` BIGINT PRIMARY KEY AUTO_INCREMENT,
    `LLM_ANSWER_ID` BIGINT NOT NULL COMMENT '关联的LLM回答ID',
    `EVALUATOR_ID` BIGINT NOT NULL COMMENT '评测者ID',
    `RAW_SCORE` DECIMAL(10, 2) NOT NULL COMMENT '原始分数',
    `NORMALIZED_SCORE` DECIMAL(10, 2) NULL COMMENT '标准化分数（0-100）',
    `WEIGHTED_SCORE` DECIMAL(10, 2) NULL COMMENT '加权分数',
    `SCORE_TYPE` VARCHAR(50) NOT NULL COMMENT '分数类型（例如：客观题得分、主观题得分、综合得分）',
    `SCORING_METHOD` VARCHAR(100) NULL COMMENT '评分方法',
    `EVALUATION_ID` BIGINT NULL COMMENT '关联的评测ID',
    `SCORING_TIME` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '评分时间',
    `CREATED_BY_USER_ID` BIGINT NULL COMMENT '创建者ID',
    `COMMENTS` TEXT NULL COMMENT '评分说明',
    FOREIGN KEY (`LLM_ANSWER_ID`) REFERENCES `LLM_ANSWERS`(`ID`) ON DELETE CASCADE,
    FOREIGN KEY (`EVALUATOR_ID`) REFERENCES `EVALUATORS`(`ID`) ON DELETE RESTRICT,
    FOREIGN KEY (`EVALUATION_ID`) REFERENCES `EVALUATIONS`(`ID`) ON DELETE SET NULL,
    FOREIGN KEY (`CREATED_BY_USER_ID`) REFERENCES `USERS`(`ID`) ON DELETE SET NULL,
    UNIQUE KEY `UK_ANSWER_SCORE` (`LLM_ANSWER_ID`, `EVALUATOR_ID`, `SCORE_TYPE`) COMMENT '确保同一回答同一评测者同一类型只有一个分数',
    INDEX `IDX_ANSWER_SCORES_ANSWER` (`LLM_ANSWER_ID`),
    INDEX `IDX_ANSWER_SCORES_EVALUATOR` (`EVALUATOR_ID`),
    INDEX `IDX_ANSWER_SCORES_TYPE` (`SCORE_TYPE`)
) ENGINE=INNODB DEFAULT CHARSET=UTF8MB4 COLLATE=UTF8MB4_UNICODE_CI;
...
CREATE TABLE `EVALUATION_RUNS` (
    ...
    `RESUME_COUNT` INT NOT NULL DEFAULT 0 COMMENT '重启次数',
    `COMPLETED_AT` DATETIME NULL COMMENT '完成时间',
    `LAST_CHECKPOINT_ID` BIGINT NULL COMMENT '最近的检查点ID',
...
-- 添加外键约束 (修改约束名称避免重复)
ALTER TABLE `EVALUATIONS`
    ADD CONSTRAINT `FK_EVALUATIONS_TO_EVAL_RUN` FOREIGN KEY (
        `EVALUATION_RUN_ID`
    )
        REFERENCES `EVALUATION_RUNS`(
            `ID`
...
-- 34. evaluation_run_checkpoints (评测运行检查点表)
DROP TABLE IF EXISTS `EVALUATION_RUN_CHECKPOINTS`;

CREATE TABLE `EVALUATION_RUN_CHECKPOINTS` (
    `ID` BIGINT PRIMARY KEY AUTO_INCREMENT,
    `EVALUATION_RUN_ID` BIGINT NOT NULL COMMENT '评测运行ID',
    `CHECKPOINT_TIME` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '检查点创建时间',
    `LAST_PROCESSED_ANSWER_ID` BIGINT NULL COMMENT '上次处理到的回答ID',
    `PROGRESS_PERCENTAGE` DECIMAL(5, 2) NULL COMMENT '当前进度百分比',
    `CHECKPOINT_DATA` JSON NULL COMMENT '检查点详细数据',
    `STATUS_BEFORE_CHECKPOINT` VARCHAR(50) NULL COMMENT '检查点前的状态',
    `NOTES` TEXT NULL COMMENT '备注信息',
    `CREATED_BY_USER_ID` BIGINT NULL COMMENT '创建检查点的用户ID',
    FOREIGN KEY (`EVALUATION_RUN_ID`) REFERENCES `EVALUATION_RUNS`(`ID`) ON DELETE CASCADE,
    FOREIGN KEY (`CREATED_BY_USER_ID`) REFERENCES `USERS`(`ID`) ON DELETE SET NULL,
    INDEX `IDX_EVAL_CHECKPOINTS_RUN_TIME` (`EVALUATION_RUN_ID`, `CHECKPOINT_TIME`)
) ENGINE=INNODB DEFAULT CHARSET=UTF8MB4 COLLATE=UTF8MB4_UNICODE_CI;

-- 添加EVALUATION_RUNS表的LAST_CHECKPOINT_ID外键约束
ALTER TABLE `EVALUATION_RUNS`
    ADD CONSTRAINT `FK_EVALUATION_RUNS_LAST_CHECKPOINT` FOREIGN KEY (
        `LAST_CHECKPOINT_ID`
    )
        REFERENCES `EVALUATION_RUN_CHECKPOINTS`(
            `ID`
        ) ON DELETE SET NULL;
